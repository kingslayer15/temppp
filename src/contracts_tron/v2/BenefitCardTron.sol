...
import "../interfaces/IWinkLink.sol";

contract BenefitCardTron is OwnableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function initialize(address _owner, address _payee_, uint256 _mintAmount, address _freeCity, address _erc1155Card, address _erc1155Vip) public initializer {
        ...
        _payee = _payee_;
        lowerPos = 9990;
        payToken = 0x7870Aa94008854268d34dc74FDD38D58cDAB59d4;
        //priceFeed = AggregatorInterface(TYvj7PaHUrPLC1vhjgL9PGvJ5FyA931KqM);
    }
    ...
    function preSale(uint256 numberOfTokens) external payable {
        ...
        //判断tick
        uint256 mintPrice = getMintPrice(msg.sender, numberOfTokens);
        uint256 allowance = IERC20(payToken).allowance(
            msg.sender,
            address(this)
        );
        require(_verifyTick(mintPrice, allowance), "verify tick error");
        ...
        emit BuyEvent(msg.sender, numberOfTokens, currentRound, mintPrice);
    }
    ...
    function getMintPrice(address account, uint256 numberOfTokens) public view returns (uint256) {
        ...
        if (numberOfTokens == 1) {
            //
            uint256 baseRPrice = getRoundBasePrice(currentRound) * discount / 100;
            uint256 resPrice = baseRPrice.mul(getLatestPrice());
            return resPrice;
        } else {
            ...
            for (uint256 i = 0; i < length; i++) {
                ...
                if (minNonce <= range && maxNonce <= range) {
                    //
                    uint256 baseRPrice = price1 * numberOfTokens * discount / 100;
                    uint256 resPrice = baseRPrice.mul(getLatestPrice());
                    return resPrice;
                    ...
                    uint256 baseRPrice = totalPrice * discount / 100;
                    uint256 resPrice = baseRPrice.mul(getLatestPrice());
                    return resPrice;
                }
            }
    ...
    function getRoundBasePrice(uint256 _round) private view returns (uint256) {
        ...
        uint256 resPrice = price2.mul(getLatestPrice());
        return resPrice;
    }
    ...
    function setLowerPos(uint256 _lowerPos) external onlyRole(OPERATOR_ROLE) {
        lowerPos = _lowerPos;
    }

    function setPriceFeed(address _priceFeed) external onlyRole(OPERATOR_ROLE) {
        priceFeed = AggregatorInterface(_priceFeed);
    }

    function setPayToken(address _payToken) external onlyRole(OPERATOR_ROLE) {
        payToken = _payToken;
    }
    ...
    function getLatestPrice() public view returns (uint256) {
        ...
        return uint256(priceFeed.latestAnswer()).div(10**6);
    }

    function _verifyTick(uint256 _curTick, uint256 _payValue) internal view returns(bool){
        if (_payValue >= _curTick){
            return true;
        }else{
            uint256 curPos = _curTick.mul(lowerPos).div(10000);
            return _payValue  >= curPos;
        }
    }
